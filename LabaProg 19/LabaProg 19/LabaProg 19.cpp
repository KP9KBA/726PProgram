...
int main() {
	...
	case 5: {
		...
		c = M;
		b = new int [M * 2 - 1];
		for (k = 0; k < M*2 - 1; k++) {//количество диагоналей
			for (i = 0; i < M; i++) {
				for (j = 0; j < M; j++) {
					if (i == j + c - 1) {//условие при котором будут использоваться нужные индексы (слева на право) 
						sum += a[i][j];
					}
				}
			}
		...
		for (i = 0; i < M*2-1; i++) {//вывод измененной матрицы
				cout << b[i] << " ";
			cout << endl;
		}
		delete[]a;
